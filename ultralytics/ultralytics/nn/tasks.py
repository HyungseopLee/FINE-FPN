...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for idx, m in enumerate(self.model):
            ...
            if isinstance(m, SpatialAlignTransnormer) or isinstance(m, SpatialAlignCrossAttentionCosFormer): 
                feat_high = x[0]
                feat_low = x[1]
                ...
                feat_low_sa, feat_high_up = m(feat_low, feat_high, idx=idx)
                
                # # Fusion Factor
                # feat_low_sa = low_FF * feat_low_sa
                # feat_high_up = high_FF * feat_high_up
                
                x = torch.cat([feat_high_up, feat_low_sa], dim=1)
                
                
            elif isinstance(m, FaPN):
                feat_high = x[0]
                feat_low = x[1]
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif m in frozenset({SpatialAlignCrossAttentionReLULinear, FaPN, SpatialAlignCrossAttentionCosFormer, SpatialAlignTransnormer}) :
