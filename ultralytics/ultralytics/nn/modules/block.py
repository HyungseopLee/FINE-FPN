...
class FaPN(nn.Module):
    def __init__(self, c1, c2, norm="GN"):
        ...
        self.offset  = nn.Sequential(
            nn.Conv2d(c1 + c2, c2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(c2),
        )
    ...
    def forward(self, feat_high, feat_low):
        ...
        offset = self.offset(torch.cat([feat_low_fsm, upsample_feat_high], dim=1))
        feat_high_align = self.feature_align([upsample_feat_high, offset])
...
# 2025.06.25 @HyungseopLee
# AlignSeg implementation: https://arxiv.org/pdf/2003.00872#page=4.10
