...
class VOCDetection(torchvision.datasets.VOCDetection, DetDataset):
    ...
    def __init__(self, root: str, ann_file: str = "train_val.txt", label_file: str = "label_list.txt", transforms: Optional[Callable] = None, return_masks=False):
        ...
        self.images = [x.replace('/ImagesSets/Main', '') for x in self.images]
        self.targets = [x.replace('/ImagesSets/Main', '') for x in self.targets]
